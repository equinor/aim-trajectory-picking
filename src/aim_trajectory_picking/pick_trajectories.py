...
def calculate_or_read_results(algos, _datasets, *, filename='results.txt', _dataset_names=None):
    ...
    if _dataset_names == None:
        dataset_names = [i for i in range(len(_datasets))]
    else:
    ...
    for data in _datasets:
        ...
        for algorithm in algos:
            pair = algorithm.__name__ + data_name
            if pair in prev_results:
                combined_results[algorithm.__name__].append(prev_results[pair])
    ...
    for name in algos:
        for result in combined_results[name.__name__]:
            assert func.check_for_collisions(result['trajectories']) == False

    JSON_IO.write_value_trajectories_runtime_from_file(filename, combined_results)
...
if __name__ == '__main__':
    ...
    plot_performances(algos, results, data_names)
